...
class TestMultiFieldModelBackend(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = User.objects.create_user(
            "test", "test@example.com", "test@123", "test", "1234567890"
        )
    ...
    def test_authenticate_with_mobile(self):
        self.assertEqual(
            authenticate(username="1234567890", password="test@123"), self.user
        )
    ...
    def test_authenticate_with_email(self):
        self.assertEqual(
            authenticate(username="test@example.com", password="test@123"), self.user
        )
